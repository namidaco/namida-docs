---
type: concept
title: Theme
description: "The overall vibe of your player"
source: "https://namidaco.github.io/namida_docs/v2/settings/1-theme-settings/"
path: /v2/settings/1-theme-settings/
version: v2
updated: 2026-08-28
okf:
  generated_by: "@docmd/plugin-okf"
  generated_at: "2026-08-28T12:46:29.902Z"
---
---
title: "Theme"
description: "The overall vibe of your player"
---

# Theme

Control how Namida looks. Colors, dark mode and language.

### Theme Mode {#theme-mode}

Switch between System, Light and Dark mode. System follows your device theme.

### Auto Coloring {#auto-coloring}

Automatically pick player colors from current artwork. Each track gets its own color, applied to the player and the rest of the app.

::: callout info
Might affect performance on low end devices. You can turn it off and set a [Default Color](#default-color) instead.
:::

### Pick Colors from Device Wallpaper {#wallpaper-colors}

`Android 12+ only`

Pick colors from your device wallpaper instead of the artwork. Requires [Auto Coloring](#auto-coloring) to be on.

### Force Miniplayer to Follow Track Colors {#force-miniplayer-colors}

The miniplayer will always use the current track colors, even when Auto Coloring, wallpaper colors or Default Color say otherwise. Those still apply to the rest of the app.

### Use Pitch Black {#pitch-black}

Use pure black background in dark mode. Useful for AMOLED screens, and looks cooler.

### Default Color {#default-color}

Set a color to be used by the player when [Auto Coloring](#auto-coloring) is off, or when the current track has no artwork. There are two entries, one for light mode and one for dark mode.

### Language {#language}

Change the app language. Translations are made by the community, you can help translating Namida at the [translation repo](https://github.com/namidaco/namida-translations).

---

<sub>by @claude</sub>
